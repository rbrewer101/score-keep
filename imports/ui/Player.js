      ...
      <div key={this.props.player._id} className={itemClassName}>
        <div className='player'>
          ...
          <div className='player__actions'>
            ...
            <button
            className='button button--round' onClick={()=>{ Players.update(this.props.player._id, {$inc: { score: 1 }}); }}>+1</button>
              <button
              className='button button--round' onClick={()=>{ Players.remove(this.props.player._id); }}>X</button>
          </div>
        </div>
      </div>);
  }
